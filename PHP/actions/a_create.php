...
if($_POST){
    ...
    $mdeiaImage = $_POST["mediaImage"];
    $mediaAuthor = $_POST["mediaAuthor"];
    $ISBN = $_POST["ISBN"];
    $status = $_POST["status"];

    $sql = "INSERT INTO `media`(`kindOfMedia`,`mediaTitle`,`mediaDescription`,`mediaImage`,`mediaAuthor`,`ISBN`,`status`) VALUES ('$kindOfMedia','$mediaTitle','$mediaDescription','$mdeiaImage','$mediaAuthor','$ISBN','$status')";
    

    if(mysqli_query($conn, $sql)){
        echo '<div class="container-fluid text-center p-5">
        <h1 class="text-info p-5 m-5">Successfully created!!</h1>
